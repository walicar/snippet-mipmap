export const mipCount = (width: number, height: number) => Math.log2(Math.max(width, height)) + 1;
...
export const bilinearFilter = (tl: Color, tr: Color, bl: Color, br: Color, t1: number, t2: number): Color => {
    const lerp = (a: number, b: number, t: number) => a + (b - a) * t;
    const mix = (a: Color, b: Color, t: number) => a.map((v, i) => lerp(v, b[i], t));
